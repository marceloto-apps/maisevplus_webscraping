"""
scripts/test_5_missing_ou.py

A diagnostic test script to select 5 matches that are missing Over/Under odds
in the database, run the updated FlashscoreOddsCollector on them (collecting stats first),
and insert the odds.
"""
import asyncio
import os
...
configure_logging()
logger = get_logger("test_5_missing_ou")
...
async def get_test_matches(pool):
    async with pool.acquire() as conn:
        ...
        # Se achou menos que 5, busca no banco geral de matches + odds_history
        needed = 5 - len(matches)
        if needed > 0:
...
async def main():
    ...
    try:
        ...
        # Inicializa o Camoufox e o collector
        metrics = CollectionMetrics()
        # Coletando apenas Over/Under (FT/HT) para o teste
        collector = FlashscoreOddsCollector(markets=["ou_ft", "ou_ht"])
        
        logger.info("\nIniciando raspagem de estatísticas e Over/Under...")
                ...
                async with pool.acquire() as conn:
                    try:
                        ...
                        result = await collector.collect_match(
                            ...
                            is_closing=False,
                            job_id="test_missing_ou_run_5",
                            metrics=metrics,
                            kickoff=m["kickoff"],
                            skip_stats=False  # Coleta as estatísticas antes das odds!
                        )
        ...
        logger.info("\n" + "="*40)
        logger.info("FIM DO TESTE DE 5 PARTIDAS (STATS + OVER/UNDER)")
