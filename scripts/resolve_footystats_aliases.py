"""
scripts/resolve_footystats_aliases.py

Resolve aliases pendentes da fonte 'footystats' lendo a tabela unknown_aliases.
...
Uso:
  python scripts/resolve_footystats_aliases.py
  python scripts/resolve_footystats_aliases.py --dry-run   # só mostra, não salva
"""
...
async def save_alias(pool, team_id: int, alias_name: str, dry_run: bool) -> None:
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO team_aliases (team_id, source, alias_name) VALUES ($1, 'footystats', $2) ON CONFLICT DO NOTHING",
            team_id, alias_name,
        )
        await conn.execute(
            "DELETE FROM unknown_aliases WHERE source = 'footystats' AND raw_name = $1",
            alias_name,
        )
...
async def main():
    ...
    async with pool.acquire() as conn:
        unknowns = await conn.fetch(
            "SELECT raw_name FROM unknown_aliases WHERE source = 'footystats' AND resolved = FALSE ORDER BY raw_name ASC"
        )
    api_teams = [r["raw_name"] for r in unknowns]

    all_teams = await get_all_db_teams(pool)
    existing = await get_existing_aliases(pool, "footystats")

    print("\n" + "=" * 65)
    print("  RESOLUTOR DE ALIASES — FOOTYSTATS")
    print("=" * 65)
    print(f"  Times no DB:      {len(all_teams)}")
    print(f"  Aliases FS:       {len(existing)}")
    ...
    if not api_teams:
        print("Nenhum alias pendente da FootyStats!")
        return
    ...
    for raw_name in api_teams:
        ...
        # 1. Correspondência exata com name_canonical
        exact_canonical = next(
            (t for t in all_teams if t["name_canonical"].lower() == raw_lower), None
        )
        if exact_canonical:
        ...
        alias_match = next(
            (t for t in all_teams if any(a.lower() == raw_lower for a in t["search_names"])), None
        )
        if alias_match:
